...
namespace AppEscritorioRHM.Core.Services
{
    ...
    public class WPService : IWPService
    {
        ...
        {
            var imageWp = await GetResourceAsync<AttachmentWpDTO>($"{WCEndpoints.ImagesMedia}/{id}", null, ct);
        ...
        {
            var imageWp = await DeleteResourceAsync<AttachmentWpDTO>($"{WCEndpoints.ImagesMedia}/{id}?force={force}", ct);
            return MapToDomain(imageWp);
        }
            ...
            try
            {
                var testConnectionTask = Task.Run(async () =>
                {
                    try
                    {
                        ...
                        var response = await GetResourceAsync<List<AttachmentWpDTO>>(WCEndpoints.ImagesMedia, parametros);
                        return true;
                    }
                    catch
                    {
