...
namespace AppEscritorioRHM.Infrastructure.ExternalServices.Woo
{
    public class WCService : IWCService
    {
        ...
        private static readonly Endpoints Endpoints = WooCommerce.GetEndpoint(WooCommerce.EndpointOptions.WooCommerce);
        ...
        {
            var productWoo = await GetResourceAsync<ProductWooDTO>($"{Endpoints.Products}/{id}", null, ct);
        ...
        {
            var productWoo = await DeleteResourceAsync<ProductWooDTO>($"{Endpoints.Products}/{id}?force={force}", ct);
            return MapProductWooToDomain(productWoo);
        }
        ...
        public async Task<List<Product>> GetProductsByPageAsync(int page, int perPage = 100, CancellationToken ct = default)
        {
            ...
            var productsWoo = await GetResourceAsync<List<ProductWooDTO>>(Endpoints.Products, parameters, ct);
        ...
        {
            var categoryWoo = await GetResourceAsync<CategoryWooDTO>($"{Endpoints.Categories}/{id}", null, ct);
            return MapCategoryWooToDTO(categoryWoo);
        }

        public async Task<List<Category>> GetCategoriesAsync(CancellationToken ct = default)
        {
            var categories = await GetResourceAsync<List<CategoryWooDTO>>(Endpoints.Categories, null, ct);
        ...
        {
            string endpoint = $"{Endpoints.Products}/{parentId}/{Endpoints.Variations}";
        ...
        {
            string endpoint = $"{Endpoints.Products}/{parentId}/{Endpoints.Variations}/{id}";
            ...
            try
            {
                var testConnectionTask = Task.Run(async () =>
                {
                    try
                    {
                        ...
                        var result = await GetResourceAsync<List<ProductWooDTO>>(Endpoints.Products, parametros);
                        return true;
                    }
                    catch
                    {
